...
> Проект реорганизован: два `.ino` файла заменены на чистые C++ модули в `src/modules`.
> Конфигурация вынесена в `src/config.h`, поддерживается инкапсуляция каждомодульных
> компонентов (датчики, дисплей, индикатор, кнопка, часы). Проект готов для сборки в
> PlatformIO без `.ino` расширений.
...
- все пункты меню переведены на английский язык в старых вариантах прошивки.
  По умолчанию язык интерфейса — русский; переключение языка удалено.
...
- Все пункты меню по умолчанию на русском; переключение языка удалено
