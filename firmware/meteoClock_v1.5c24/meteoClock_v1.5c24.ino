...
#define SEALEVELPRESSURE_HPA (1020.25)  // Коэффициент для расчета высоты над уровнем моря
...
#define BRIGHT_THRESHOLD 300  // величина сигнала, ниже которой яркость переключится на минимум (0-1023)
#define BRIGHT_HYSTERESYS 50  // Гистерезис устраняет "дребезг" на границе свет/тень и делает автоматическую яркость стабильной.
...
#define LCD_BRIGHT_MIN 60     // мин яркость подсветки дисплея (0 - 255)
...
#define TEMP_OFFSET (-5.5)  // поправка на температуру (реальная = измеренная + TEMP_OFFSET)
